		...
		<div class="span10">
			<table class="table table-bordered table-hover">
				...
				<tbody>
					...
					<c:forEach items="${showWebUserLogBeanList}" var="showWebUserLogBeanList" varStatus="status">
					  <!-- 选择颜色操作 -->
					  <c:choose>
					  ...
					  <!-- 遍历集合操作 -->					 
					  <tr ${color}>
					  	<td>${showWebUserLogBeanList.associationUserLog.user_name }</td>						
						<td>
							<c:if test="${showWebUserLogBeanList.op_type=='1'}">
								登录
							</c:if>
							<c:if test="${showWebUserLogBeanList.op_type=='2'}">
								退出
							</c:if>
						</td>
						<td>${showWebUserLogBeanList.created_at}</td>
						<!--  <td>${showWebUserLogBeanList.associationUserLog.current_sign_in_ip}</td> -->
					  </tr>					  
					</c:forEach>
